...
COUNTRIES = (
    "افغانستان|آلبانی|الجزایر|آرژانتین|استرالیا|اتریش|بلژیک|برزیل|بلغارستان|کانادا|"
    "شیلی|چین|کلمبیا|کرواسی|کوبا|دانمارک|مصر|فنلاند|فرانسه|آلمان|"
    "یونان|هند|اندونزی|ایران|عراق|ایرلند|ایتالیا|ژاپن|قزاقستان|کنیا|"
    "کره جنوبی|مکزیک|مراکش|هلند|نیوزیلند|نروژ|پاکستان|پرو|لهستان|پرتغال|"
    "رومانی|روسیه|عربستان سعودی|صربستان|سنگاپور|اسپانیا|سوئد|سوئیس|ترکیه|اوکراین|"
    "امارات متحده عربی|بریتانیا|ایالات متحده|ونزوئلا|ویتنام|آفریقای جنوبی|مصر|مغولستان|تایلند|مالزی|"
    "فیلیپین|چک|اسلواکی|مجارستان|بلاروس|گرجستان|ارمنستان|آذربایجان|قطر|کویت|"
    "اردن|لبنان|عمان|بحرین|تونس|لیبی|اتیوپی|نیجریه|غنا|کنیا|"
    "تانزانیا|اوگاندا|آنگولا|موزامبیک|زیمبابوه|شیلی|بولیوی|اکوادور|اروگوئه|پاراگوئه|"
    "کاستاریکا|پاناما|کوبا|جامائیکا|ایسلند|لوکزامبورگ|اسلوونی|کره شمالی|اسرائیل"
).split("|")
...
async def start(m: Message):
    async with Session() as s:
        ...
        text = (
            f"🌍 <b>WORLD WAR</b>\n\n"
            f"کشور: <b>{p.country}</b>\n"
            f"💰 خزانه: {fmt(p.money)}\n\n"
            f"یک بخش را انتخاب کن."
        )
        await m.answer(text, reply_markup=menu(), parse_mode="HTML")
...
async def callbacks(c: CallbackQuery):
    async with Session() as s:
        ...
        if d == "assets":
            text = (
                f"💼 <b>دارایی‌های {p.country}</b>\n\n"
                f"💰 {fmt(p.money)}\n"
                f"👥 جمعیت: {p.population:,}\n"
                f"🪖 زمینی: {p.land:,}\n"
                f"✈️ هوایی: {p.air:,}\n"
                f"🚢 دریایی: {p.navy:,}\n"
                f"🛡️ پدافند: {p.defense:,}\n"
                f"🎯 استراتژیک: {p.strategic:,}\n"
                f"🛡️ تانک: {p.tanks:,}\n"
                f"💥 توپخانه: {p.artillery:,}\n"
                f"🚀 موشک: {p.missiles:,}\n"
                f"☢️ هسته‌ای: {p.nuclear:,}\n"
                f"🏭 کارخانه: {p.factories}\n"
                f"🚗 خودرو: {p.auto_factories}\n"
                f"⛏️ معدن: {p.mines}\n"
                f"🛢️ نفت: {p.oil}\n"
                f"🏨 هتل: {p.hotels}\n"
                f"🚢 بندر: {p.ports}\n"
            ...
            income = (
                ...
                + p.factories * 100000
                + p.hotels * 25000
                + p.auto_factories * 70000
            )
            text = f"💰 <b>اقتصاد</b>\nدرآمد روزانه تقریبی: {fmt(income)}\n\nمنابع و صنعت روی درآمد اثر می‌گذارند."
            await c.message.edit_text(text, reply_markup=menu(), parse_mode="HTML")
            ...
            items = [
                ...
                ("🚢 +1 دریایی", "buy:navy"),
                ("🛡️ +1 پدافند", "buy:def"),
                ("🎯 +1 استراتژیک", "buy:strat"),
                ("🛡️ +10 تانک", "buy:tank"),
                ("💥 +10 توپخانه", "buy:art"),
                ("🚀 +1 موشک", "buy:missile"),
                ("☢️ +1 آیتم هسته‌ای", "buy:nuke"),
            ]
            for t, x in items:
                b.button(text=t, callback_data=x)
            b.adjust(2)
            text = "🪖 <b>مدیریت نیروها</b>\nهمه موارد در این پروژه آیتم‌های بازی هستند."
            await c.message.edit_text(text, reply_markup=b.as_markup(), parse_mode="HTML")

        elif d == "build":
            b = InlineKeyboardBuilder()
            items = [
                ("🏭 کارخانه", "build:factory"),
                ("🚗 صنعت خودرو", "build:auto"),
                ("⛏️ معدن", "build:mine"),
                ("🛢️ نفت", "build:oil"),
                ("🏨 هتل", "build:hotel"),
                ("🚢 بندر", "build:port"),
                ("✈️ فرودگاه", "build:airport"),
                ("🏰 پایگاه", "build:base"),
            ]
            for t, x in items:
                b.button(text=t, callback_data=x)
            b.adjust(2)
            await c.message.edit_text("🏭 <b>ساخت‌وساز</b>", reply_markup=b.as_markup(), parse_mode="HTML")
            ...
            b.adjust(1)
            text = "⚔️ <b>جنگ و دیپلماسی</b>\nبرای هدف از دستور /war استفاده کن."
            await c.message.edit_text(text, reply_markup=b.as_markup(), parse_mode="HTML")

        elif d == "statement":
            await c.message.answer("📜 /statement متن بیانیه")
            ...
            await c.message.answer("🏦 /loan مبلغ — سقف 100,000,000 و سررسید 2 روز")

        elif d in ("sea", "air"):
            mode = "دریایی" if d == "sea" else "هوایی"
            icon = "🚢" if d == "sea" else "✈️"
            await c.message.answer(f"{icon} /trade {mode} @username مبلغ")

        elif d == "invite":
            me = await c.bot.me()
            text = (
                f"👥 لینک دعوت:\n"
                f"https://t.me/{me.username}?start=ref_{p.tg}\n\n"
                f"پاداش دعوت را می‌توان در مرحله بعد تنظیم کرد."
            )
            await c.message.edit_text(text, reply_markup=menu())
            ...
            b.adjust(3)
            await c.message.edit_text("🌍 کشور جدید را انتخاب کن:", reply_markup=b.as_markup())
            ...
            if taken and taken.tg != p.tg:
                await c.answer("این کشور قبلاً گرفته شده.", show_alert=True)
            else:
                p.country = name
                await s.commit()
                text = f"✅ کشور انتخاب شد: <b>{name}</b>"
                await c.message.edit_text(text, reply_markup=menu(), parse_mode="HTML")

        elif d.startswith("buy:"):
            k = d.split(":")[1]
            vals = {
                "land": (100000, "land", 100),
                "air": (500000, "air", 1),
                "navy": (1200000, "navy", 1),
                "def": (800000, "defense", 1),
                "strat": (1500000, "strategic", 1),
                "tank": (1000000, "tanks", 10),
                "art": (700000, "artillery", 10),
                "missile": (2500000, "missiles", 1),
                "nuke": (20000000, "nuclear", 1),
            }
            cost, attr, q = vals[k]
            if p.money < cost:
                await c.answer("بودجه کافی نیست.", show_alert=True)
            else:
                p.money -= cost
                setattr(p, attr, getattr(p, attr) + q)
                await s.commit()
                await c.answer("خرید انجام شد.")

        elif d.startswith("build:"):
            k = d.split(":")[1]
            vals = {
                "factory": (5000000, "factories"),
                "auto": (10000000, "auto_factories"),
                "mine": (8000000, "mines"),
                "oil": (15000000, "oil"),
                "hotel": (1500000, "hotels"),
                "port": (12000000, "ports"),
                "airport": (10000000, "airports"),
                "base": (20000000, "bases"),
            }
            cost, attr = vals[k]
            if p.money < cost:
                ...
                await s.commit()
                await c.answer("ساخت انجام شد.")

        elif d == "countries":
            await c.message.answer("🌍 برای دیدن/انتخاب کشورها از «تغییر کشور» استفاده کن.")

        elif d == "ally:create":
            await c.message.answer("/alliance نام_اتحاد")

        elif d == "ally:friendly":
            await c.message.answer("/friendly کشور")

        elif d == "ally:list":
            aa = (await s.execute(select(Alliance))).scalars().all()
            lines = [f"🤝 {a.name} {'💙' if a.friendly else '⚔️'}" for a in aa]
            await c.message.answer("\n".join(lines) or "اتحادی وجود ندارد.")

        elif d == "war:declare":
            await c.message.answer("/war @username")

        elif d == "war:peace":
            await c.message.answer("/peace @username")
...
async def statement(m):
    ...
    if not text:
        return await m.answer("/statement متن")
    async with Session() as s:
        p = await player(s, m.from_user.id, m.from_user.username)
...
async def commander(m):
    ...
    if not name:
        return await m.answer("/commander نام")
    async with Session() as s:
        p = await player(s, m.from_user.id, m.from_user.username)
        p.commander = name
        await s.commit()
...
async def alliance(m):
    name = m.text.partition(" ")[2].strip()[:100]
    if not name:
        return await m.answer("/alliance نام")
    async with Session() as s:
        p = await player(s, m.from_user.id, m.from_user.username)
        a = Alliance(name=name, owner=p.tg)
        s.add(a)
        await s.flush()
        p.alliance_id = a.id
        await s.commit()
...
async def friendly(m):
    text = "💙 پیشنهاد اتحاد دوستانه ثبت شد. نسخه بعدی می‌تواند تأیید دوطرفه و مدت قرارداد را اضافه کند."
    await m.answer(text)


async def loan(m):
    try:
        amount = Decimal(m.text.split()[1])
    except Exception:
        return await m.answer("/loan 10000000")
    if amount <= 0 or amount > 100000000:
        return await m.answer("حداکثر وام: 100,000,000")
    async with Session() as s:
        p = await player(s, m.from_user.id, m.from_user.username)
        p.money += amount
        s.add(Loan(borrower=p.tg, amount=amount, due=datetime.now(timezone.utc) + timedelta(days=2)))
        await s.commit()
...
async def income_loop():
    while True:
        try:
            async with Session() as s:
                ps = (await s.execute(select(Player))).scalars().all()
